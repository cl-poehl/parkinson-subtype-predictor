...
def _imputer_comparison_panel():
    ...
    st.caption(
        ...
        "bootstrap 95% CI. All differences are within +/-0.013 AUC with "
        "overlapping CIs -- the choice of imputer is **not statistically "
        "significant**.\n\n"
        "**Why kNN (k=5) is the deployed default**, a stack of arguments "
        "(in decreasing strength):\n"
        "1. *No class-imbalance bias*: PPMI is 4.5:1 slow:fast; global "
        "median/mean systematically shift Fast features toward the Slow "
        "distribution. kNN is patient-aware and avoids this.\n"
        "2. *Deterministic*: identical input -> identical output. MICE and "
        "missForest depend on `random_state` and a convergence loop; their "
        "outputs vary across implementations.\n"
        "3. *Inspectable*: for each imputed value we can show the 5 PPMI "
        "patients that contributed -- MICE/SoftImpute produce opaque "
        "imputations.\n"
        "4. *Parsimonious*: one hyperparameter (k=5) vs MICE's many "
        "(n_iterations, estimator, sample_posterior, ...).\n"
        "5. *Sample-efficient*: missingness-indicator variants add N "
        "extra binary features, hurting EPV at n=409.\n"
        "6. *Avoids low-rank assumption* (vs SoftImpute) which is fragile "
        "with heterogeneous PPMI missingness patterns.\n"
        "7. *Empirical equivalence*: the table above confirms kNN is "
        "neither better nor worse than the alternatives, so we choose on "
