...
population-level latent-time model is required.
...
(free tier, so the first load after idle can take a minute to wake up)
...
Mean score trajectories by predicted subtype, showing face validity that the
predicted label tracks real progression, not just baseline level:
